...
function convertToSQLTimestamp(dateStr) {
    ...
    // Construct the SQL timestamp string in the format 'YYYY-MM-DD HH:MM:SS'
...
function updateUserDetails(userDets) {
	...
	document.getElementById("empJoinDate").innerText = convertDateFormat(
		userDets.datejoined
	);
	document.getElementById("empAge").innerText = calculateAge(
		userDets.dateofbirth
	);
...
function formatDateForPostgres(date) {
	...
	return `${dayOfWeek} ${month} ${dayOfMonth} ${year} ${hours}:${minutes}:${seconds} GMT${timezone}${timezoneOffsetHours}${timezoneOffsetMinutes} (${
...
async function addItemToDB() {
	...
	const detailsUploadStatus = await putItemsToDB(
		itemID,
		itemQuantity,
		itemName,
		itemPrice
...
function addRowInTable() {
	var saleItemID = document.getElementById("saleItemID").value;
	var saleItemQuantity = document.getElementById("saleItemQuantity").value;
	var saleItemName = document.getElementById("itemNameSale").innerText;
	var saleItemPrice = document.getElementById("valueSold").innerText;
	...
	cell1.innerHTML = saleItemID;
	cell2.innerHTML = saleItemName;
	cell3.innerHTML = saleItemQuantity;
	cell4.innerHTML = saleItemPrice;

	document.getElementById("addSalesForm").reset();
}
function updateTotalPrice(){
    console.log(document.getElementById('saleItemQuantity').value*ip);
    document.getElementById('valueSold').innerText=ip*document.getElementById('saleItemQuantity').value;
}
var ip;
async function fetchItemFromDB() {
	const itemID = document.getElementById("saleItemID").value;
	const cartItem = await getItemFromDB(itemID);
    console.log(cartItem[0].product_name);
    document.getElementById('itemNameSale').innerText=cartItem[0].product_name;
    ip=cartItem[0].product_price;
}

const getItemFromDB = async (itemID) => {
	const response = await fetch("http://localhost:3000/api/fetch/item", {
		method: "GET",
		headers: {
			product_id: itemID
		},
...
async function addToCart() {
	const product_id = document.getElementById("saleItemID").value;
	const num_items_sold = document.getElementById("saleItemQuantity").value;
...
const putItemsToSaleDB = async (product_id, num_items_sold,sale_date, sale_amount) => {
